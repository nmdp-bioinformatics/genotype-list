...
/**
 * Implementation of HttpClient based on RestAssured with OAuth 2.0 bearer token support.
 */
public final class RestAssuredOAuthHttpClient implements HttpClient {
    private final Header header;
    private final Logger logger = LoggerFactory.getLogger(getClass());

    /**
     * Create a new HTTP client with the specified OAuth 2.0 bearer token.
     *
     * @param bearerToken OAuth 2.0 bearer token, must not be null
     */
    @Inject
    public RestAssuredOAuthHttpClient(@BearerToken final String bearerToken) {
        checkNotNull(bearerToken);
        header = new Header("Authentication", "Bearer " + bearerToken);
    }

    @Override
    public InputStream get(final String url) {
        long start = System.nanoTime();
        Response response = RestAssured.with().header(header).get(url);
    ...
    public String post(final String url, final String body) throws HttpClientException {
        long start = System.nanoTime();
        Response response = RestAssured.with().header(header).body(body).contentType("text/plain").post(url);
        long elapsed = System.nanoTime() - start;
        int statusCode = response.statusCode();
        if (logger.isTraceEnabled()) {
