 ...
 * Accession sort.
 */
public final class AccessionSort extends AbstractSortTask {
    private static final String USAGE = "accession-sort [-i identifiers.txt] [-o sorted-glstrings.txt]";

    /**
     * Create a new accession sort task.
     *
     * @param client gl client, must not be null
     */
    public AccessionSort(final GlClient client) {
    ...
    protected String sort(final AlleleList alleleList) {
        ...
        Collections.sort(alleles, AlleleComparators.BY_ACCESSION_ASCENDING);
        ...
        try
        {
            ...
            if (help.wasFound()) {
                ...
                if (!(outputIdentifiers.wasFound() || outputOriginalGlstrings.wasFound() || outputSortedGlstrings.wasFound())) {
                    ...
                    new AccessionSort(client, identifierFile.getValue(), sortedGlstringFile.getValue(), false, false, true).run();
                }
                else {
                    new AccessionSort(client, identifierFile.getValue(), sortedGlstringFile.getValue(), outputIdentifiers.wasFound(),
