...
*/
package org.nmdp.gl.service.id;
...
/**
 * Base 36 identifier supplier.
 */
@Immutable
final class Base36IdSupplier implements IdSupplier {
    ...
    @Inject
    Base36IdSupplier(@Namespace final String ns) {
        checkNotNull(ns);
        this.ns = ns;
    }
    ...
    public String createLocusId() {
        return ns + "locus/" + encode(locusId.getAndIncrement());
    }

    @Override
    public String createAlleleId() {
        return ns + "allele/" + encode(alleleId.getAndIncrement());
    }

    @Override
    public String createAlleleListId() {
        return ns + "allele-list/" + encode(alleleListId.getAndIncrement());
    }

    @Override
    public String createHaplotypeId() {
        return ns + "haplotype/" + encode(haplotypeId.getAndIncrement());
    }

    @Override
    public String createGenotypeId() {
        return ns + "genotype/" + encode(genotypeId.getAndIncrement());
    }

    @Override
    public String createGenotypeListId() {
        return ns + "genotype-list/" + encode(genotypeListId.getAndIncrement());
    }

    @Override
    public String createMultilocusUnphasedGenotypeId() {
        return ns + "multilocus-unphased-genotype/" + encode(multilocusUnphasedGenotypeId.getAndIncrement());
    }
